...
CREATE TABLE IF NOT EXISTS `current_vote_stack` (
  ...
  PRIMARY KEY (`id_hash`),
  FOREIGN KEY (`song1_id`) REFERENCES `songs`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (`song2_id`) REFERENCES `songs`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (`song3_id`) REFERENCES `songs`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
